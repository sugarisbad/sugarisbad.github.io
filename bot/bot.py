...
async def analyze_task(text: str) -> str:
    result = await ollama_generate(
        "Ты — ассистент DevOps-инженера. Клиент оставил заявку. Твой вывод целиком "
        "показывается инженеру как есть — НЕ пиши план, рассуждения или комментарии. "
        "Первая строка вывода должна начинаться со слова «Суть:».\n"
    ...
    # мелкие модели любят преамбулу — оставляем разбор начиная с «Суть:»
    i = result.find("Суть:")
    return result[i:].strip() if i > 0 else result
...
async def draft_answer(question: str) -> str:
    result = await ollama_generate(
        ...
        "Клиент задал вопрос в Telegram-боте. Твой вывод целиком отправляется "
        "клиенту как есть — НЕ пиши план, рассуждения, заголовки или слово "
        "«Ответ», начни сразу с обращения к клиенту. По-русски, дружелюбно и "
        "коротко (до 80 слов). Не выдумывай цены и сроки — если спрашивают "
